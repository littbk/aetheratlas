...
async function connectDrive(){const clientId=$('driveClientId').value.trim();if(!clientId.endsWith('.apps.googleusercontent.com')){revealDriveSetup();driveStatus('Informe o Client ID OAuth de um Aplicativo Web para conectar.');notify('Configure o Client ID OAuth do Google Drive.');return;}$('driveConnect').disabled=true;try{await loadGoogleIdentity();localStorage.setItem('aether-atlas-drive-client-id',clientId);const token=await new Promise((resolve,reject)=>{const tc=google.accounts.oauth2.initTokenClient({client_id:clientId,scope:DRIVE_SCOPE,callback:r=>r.error?reject(Error(r.error)):resolve(r.access_token)});tc.requestAccessToken({prompt:'consent'});});driveToken=token;driveStatus(driveFileId?'Conectado. Autosave atualizará este projeto.':'Conectado. O próximo salvamento criará o arquivo.');$('driveConnect').textContent='Drive conectado';notify('Google Drive conectado.');await saveToDrive();}catch(err){revealDriveSetup();driveStatus('Não foi possível conectar: '+err.message);notify('Não foi possível conectar ao Google Drive.');}finally{$('driveConnect').disabled=false;}}
